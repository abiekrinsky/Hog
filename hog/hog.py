...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal) and (score1 < goal):
        if player == 0:
            mine, oppo, strat = score0, score1, strategy0
        else:
            mine, oppo, strat = score1, score0, strategy1
        mine += take_turn(strat(mine, oppo), oppo, dice)
        if is_swap(mine, oppo):
            mine, oppo = oppo, mine
        if player == 0:
            score0, score1 = mine, oppo
        else:
            score1, score0 = mine, oppo 
...
def make_averaged(fn, num_samples=1000):
    ...
    def average(*args):
        ...
        while number > 0:
            summ += fn(*args)
            number -= 1
        make_averaged = summ/num_samples
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    maximum = 0
    while times > 0:
        current_ex = make_averaged(roll_dice, num_samples)(times, dice)
        if maximum < current_ex:
            maximum = current_ex
            best_times = times
        times -= 1
    return best_times
